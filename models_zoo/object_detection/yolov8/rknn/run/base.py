...
class RK3588:
    ...
    def run(self, img: np.ndarray, draw=False) -> np.ndarray:
        t1 = time.time()
        ...
        t4 = time.time()
        dt1 = t2-t1 # preprocess
        dt2 = t3-t2 # inference
        dt3 = t4-t3 # post process (without draw)
        ...
        if all(x is not None for x in (boxes, classes, scores)):
            ...
            preds = np.concatenate([boxes, scores, classes], axis=1)
            return img, preds
        else:
            return img, np.array([])
            

    def main(self, source, draw=False):
        ...
        result_img, preds = self.run(frame, draw)
        boxes = preds[:,:4] 
        classes = preds[:,5]
        scores = preds[:,4]
        if draw:
            inf_img = self.draw(result_img, boxes, classes, scores) # type: ignore
            cv2.imwrite('/home/orangepi/PavelZh/res.jpg', result_img)